...
class CmaEsOptimizer(BasicOptimizer):
    '''
    ...
    def configure(self):
        ...
        self.sigma = (self.xbounds[1][0] - self.xbounds[0][0])/4
        self.popsize = 2
    ...
    def evaluate(self, x):
        ...
        self.record(x, y, z)
        self.batch_idx += 1
        return list(-y.reshape(-1))[0]

    def optimize(self):
        self.initialize()
        x_init = np.random.uniform(low=self.xbounds[0, :], high=self.xbounds[1, :], size=(1, self.xdim))
        res = cma.fmin(self.evaluate, x_init, self.sigma,
                       options={"bounds": self.xbounds.tolist(), "verbose": -1,
