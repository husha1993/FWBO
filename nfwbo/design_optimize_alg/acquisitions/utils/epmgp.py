'''
source code from:
https://github.com/amzn/emukit/blob/master/emukit/bayesian_optimization/epmgp.py
https://github.com/ProbabilisticNumerics/entropy-search/blob/master/matlab/min_factor.m
'''
...
def min_factor(Mu, Sigma, k, gamma=1):
    ...
    if np.isnan(d):
        ...
        IRSR = 0.5 * (IRSR + IRSR.T) # ensure symmetry.
        if not np.all(IRSR-IRSR.T == 0):
            raise ValueError('IRSR not symetry due to np issue')
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        cIRSR = np.linalg.cholesky(IRSR + 1e-5 * np.eye(IRSR.shape[0]))
                    except np.linalg.LinAlgError:
                        try:
                            cIRSR = np.linalg.cholesky(IRSR + 1e-4 * np.eye(IRSR.shape[0]))
                        except np.linalg.LinAlgError:
                            try:
                                cIRSR = np.linalg.cholesky(IRSR + 1e-3 * np.eye(IRSR.shape[0]))
                            except np.linalg.LinAlgError:
                                try:
                                    cIRSR = np.linalg.cholesky(IRSR + 1e-2 * np.eye(IRSR.shape[0]))
                                except np.linalg.LinAlgError:
...
def lt_factor(s, l, M, V, mp, p, gamma):
    ...
    Vc = (V[:, l] - V[:, s]) / sq2
    cM = (M[l] - M[s]) / sq2
    cVnic = np.max([cVc / (1 - p * cVc), 0+1e-25])
