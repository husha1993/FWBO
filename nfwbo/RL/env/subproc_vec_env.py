...
from RL.env import VecEnv, CloudpickleWrapper
  ...
  try:
    while True:
      ...
      if cmd == 'step':
        ...
        ob = env.reset()
        remote.send(ob)
      elif cmd == 'get_current_design':
        design = env.get_current_design()
        remote.send(design)
...
class SubprocVecEnv(VecEnv):
  ...
  def get_current_design(self):
    self._assert_not_closed()
    self.remotes[0].send(('get_current_design', None))
    design = self.remotes[0].recv()
    return design
